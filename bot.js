...
const SEARCH_COOLDOWN_MINUTES = 20; // антифлуд для аккаунта сессии
    ...
    if (isNew && payload && payload.startsWith('ref_')) {
        ...
        if (!isNaN(refId) && refId !== chatId) {
            ...
            if (res.changes > 0) {
                ...
                try {
                    await bot.sendMessage(refId, '🎉 По твоей ссылке зарегистрировался новый пользователь!');
                } catch (e) { /* пользователь мог заблокировать бота */ }
            }
        }
    }
...
const HARD_CHECK_LIMIT = 700; // предохранитель, чтобы не проверять бесконечно
...
bot.on('message', async (msg) => {
    ...
    if (state.type === 'rate') {
        ...
        const { score, feedback } = rateUsername(username);        let text = `⭐ @${username}\nОценка: ${score}/10\n\n`;
...
bot.on('callback_query', async (query) => {
    ...
    if (data === 'achievements') {
        const stats = await db.get(
            'SELECT total_searches, referral_count FROM users WHERE user_id = ?',
            chatId
        );
        ...
        if (unlocked.length) {            unlocked.forEach((a) => { text += `✅ ${a.name}\n`; });
        } else {
    ...
    if (data === 'referral') {
        ...
        const text = `🎁 Реферальная программа\n\nПриглашай друзей по своей ссылке — они попадут в счётчик приглашённых для достижений.\n\nТвоя ссылка:\n${link}\n\n👥 Приглашено: ${row ? row.referral_count : 0}`;
        await bot.editMessageText(text, {
            chat_id: chatId, message_id: messageId,
            reply_markup: { inline_keyboard: [[{ text: '◀️ Назад', callback_data: 'menu' }]] }
        });
