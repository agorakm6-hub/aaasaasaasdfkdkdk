import logging
import re
import os
from telethon.sync import TelegramClient
from telethon import functions, events

logging.basicConfig(level=logging.INFO)

BOT_TOKEN = "8895080427:AAE02i4cD0NeWVOShOC6btza4PMitpJxgk8"
API_ID = 39328144
API_HASH = "b4c02b2f6297f1b61d3073fd50629711"

user_sessions = {}
user_states = {}

bot = TelegramClient('bot', API_ID, API_HASH).start(bot_token=BOT_TOKEN)

def get_client(user_id):
    return user_sessions.get(user_id)

def kill_client(user_id):
    if user_id in user_sessions:
        try:
            user_sessions[user_id].disconnect()
        except:
            pass
        del user_sessions[user_id]

@bot.on(events.NewMessage(pattern='/start'))
def start_handler(event):
    user_id = event.sender_id
    if get_client(user_id):
        show_menu(event)
    else:
        keyboard = {
            "inline_keyboard": [
                [{"text": "🔑 Войти по .session", "callback_data": "login_file"}],
                [{"text": "🆕 Новый аккаунт", "callback_data": "new"}]
            ]
        }
        event.respond("👋 Отправь .session файл:", buttons=keyboard)

def show_menu(event, message_id=None):
    keyboard = {
        "inline_keyboard": [
            [{"text": "🗑 Удалить аккаунт", "callback_data": "del"}],
            [{"text": "📤 Отправить сообщение", "callback_data": "send"}],
            [{"text": "📖 Читать сообщения", "callback_data": "read"}],
            [{"text": "📢 Мои каналы", "callback_data": "channels"}],
            [{"text": "👥 Контакты", "callback_data": "contacts"}],
            [{"text": "⭐ Баланс звёзд", "callback_data": "stars"}],
            [{"text": "🚪 Выйти", "callback_data": "logout"}]
        ]
    }
    if message_id:
        event.edit("📱 Меню:", buttons=keyboard)
    else:
        event.respond("📱 Меню:", buttons=keyboard)

@bot.on(events.NewMessage)
def file_handler(event):
    user_id = event.sender_id
    
    if user_id in user_states and user_states[user_id] == 'awaiting_file':
        if event.message.file:
            file_name = event.message.file.name
            if not file_name.endswith('.session'):
                event.respond("❌ Отправь .session файл")
                return
            
            try:
                file_path = f"session_{user_id}.session"
                event.message.download_media(file=file_path)
                
                client = TelegramClient(file_path, API_ID, API_HASH)
                client.start()
                client.get_me()
                
                user_sessions[user_id] = client
                del user_states[user_id]
                
                event.respond("✅ Вход по .session выполнен!")
                show_menu(event)
                
            except Exception as e:
                event.respond(f"❌ Ошибка: {e}")
            finally:
                try:
                    os.remove(file_path)
                except:
                    pass

@bot.on(events.CallbackQuery)
def callback_handler(event):
    user_id = event.query.user_id
    data = event.data.decode()
    message_id = event.query.msg_id

    event.answer()

    if data == "login_file":
        user_states[user_id] = 'awaiting_file'
        event.edit("📁 Отправь .session файл:")

    elif data == "new":
        event.edit("🆕 Создай аккаунт в Telegram, потом войди через /start")

    elif data == "del":
        client = get_client(user_id)
        if not client:
            event.edit("❌ Нет сессии")
            return
        try:
            client(functions.account.DeleteAccountRequest(reason="Удаление"))
            kill_client(user_id)
            event.edit("✅ Аккаунт удалён")
        except Exception as e:
            event.edit(f"❌ Ошибка: {e}")

    elif data == "send":
        user_states[user_id] = 'awaiting_message'
        event.edit("📤 Введи: @user1 @user2 Текст сообщения")

    elif data == "read":
        client = get_client(user_id)
        if not client:
            event.edit("❌ Нет сессии")
            return
        try:
            dialogs = client.get_dialogs(limit=10)
            if not dialogs:
                event.edit("📭 Нет диалогов")
                return
            out = "📖 Последние диалоги:\n\n"
            for d in dialogs:
                out += f"• {d.name}\n"
                msgs = client.get_messages(d.id, limit=2)
                for m in msgs:
                    txt = m.text[:40] if m.text else "[медиа]"
                    out += f"  └ {txt}\n"
                out += "\n"
            event.edit(out[:4000])
        except Exception as e:
            event.edit(f"❌ Ошибка: {e}")

    elif data == "channels":
        client = get_client(user_id)
        if not client:
            event.edit("❌ Нет сессии")
            return
        try:
            dialogs = client.get_dialogs()
            chs = [d for d in dialogs if d.is_channel and d.entity.creator]
            if not chs:
                event.edit("📢 Нет созданных каналов")
                return
            out = "📢 Твои каналы:\n"
            for c in chs:
                link = f"t.me/{c.entity.username}" if c.entity.username else "без ссылки"
                out += f"• {c.name} — {link}\n"
            event.edit(out)
        except Exception as e:
            event.edit(f"❌ Ошибка: {e}")

    elif data == "contacts":
        client = get_client(user_id)
        if not client:
            event.edit("❌ Нет сессии")
            return
        try:
            contacts = client.get_contacts()
            if not contacts:
                event.edit("👥 Нет контактов")
                return
            out = "👥 Контакты:\n"
            for c in contacts[:20]:
                name = f"{c.first_name or ''} {c.last_name or ''}".strip() or "Без имени"
                out += f"• {name} | @{c.username or 'нет'} | ID: {c.id}\n"
            event.edit(out[:4000])
        except Exception as e:
            event.edit(f"❌ Ошибка: {e}")

    elif data == "stars":
        client = get_client(user_id)
        if not client:
            event.edit("❌ Нет сессии")
            return
        try:
            me = client.get_me()
            stars = getattr(me, 'stars', 0)
            event.edit(f"⭐ Баланс звёзд: {stars}")
        except Exception as e:
            event.edit(f"❌ Ошибка: {e}")

    elif data == "logout":
        kill_client(user_id)
        if user_id in user_states:
            del user_states[user_id]
        event.edit("🚪 Вышел. Используй /start для входа")

@bot.on(events.NewMessage(pattern='/help'))
def help_handler(event):
    event.respond("📋 Команды:\n/start - начать\n/help - помощь")

def main():
    print("🚀 Бот запущен!")
    bot.run_until_disconnected()

if __name__ == "__main__":
    main()