...
def check_session(session_string):
    """Проверяет сессию Pyrogram"""
    try:
        client = Client("check_sess", api_id=API_ID, api_hash=API_HASH, session_string=session_string, in_memory=True)
        client.start()
        client.get_me()
        return client
    ...
    if 'message' in data:
        ...
        if text == '/start':
            ...
            if client:
                ...
                send_message(chat_id, "❌ Сессия невалидна! Проверь строку.")
