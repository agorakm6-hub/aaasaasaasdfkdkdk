...
# ====== КЛИЕНТЫ ======
# 1. Бот (для общения с пользователями)
bot = TelegramClient('bot', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
# 2. Сессия (для проверки юзернеймов)
session = TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH)
session.start()
...
async def find_available_usernames(length):
    ...
    while len(found) < 25 and attempts < 3000:
        ...
        if len(username) == length:
            try:
                await session.get_entity(username)
...
async def admin_handler(event):
    ...
    if state == 'give':
        ...
        if match:
            ...
            try:
                entity = await session.get_entity(username)
