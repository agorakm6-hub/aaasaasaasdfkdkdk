...
async def mines_handler(event):
    ...
    # Генерация поля
    board = [0] * 25  # 0 - пусто
    mines = random.sample(range(25), 6)  # 6 мин
    for m in mines:
        board[m] = 1  # 1 - мина
...
async def callback_handler(event):
    ...
    if data == "menu_back":
        ...
        if stars in amounts:
            await event.edit(f"💎 Переведи {stars} ⭐ и получи {amounts[stars]} 🍉\n\nПосле оплаты напиши админу @arbuZOV", buttons=[[Button.inline("◀️ Назад", "donate")]])
    
    elif data == "premium_month":
        await event.edit("⭐ Переведи 15 ⭐ и получи премиум на месяц\n\nПосле оплаты напиши админу @arbuZOV", buttons=[[Button.inline("◀️ Назад", "premium")]])
