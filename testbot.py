...
def check_session_sync(session_string):
    """Проверка сессии в синхронном режиме"""
    try:
        client = TelegramClient("check_sess", API_ID, API_HASH)
...
def webhook():
    ...
    if 'message' in data:
        ...
        if text == '/start':
            ...
            session_string = text
            client = check_session_sync(session_string)
