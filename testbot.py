...
def check_session(session_string):
    """Проверяет сессию и возвращает клиент или None"""
    try:
        client = TelegramClient(f"check_sess", API_ID, API_HASH)
...
def webhook():
    ...
    if 'message' in data:
        ...
        if text == '/start':
            ...
            # ПРОВЕРЯЕМ СЕССИЮ
            session_string = text
            client = check_session(session_string)
            
            if client:
                # Сессия валидная — сохраняем и показываем меню
                ...
                # Сессия хуйня
                send_message(chat_id, "❌ Сессия невалидна! Проверь, что строка правильная.")
