...
def game_buttons(game, amount, bet_type=None):
    if game == "roulette":
        return [
            [Button.inline("🔄 Удвоить", f"double_r_{amount}_{bet_type}"), Button.inline("🔁 Повторить", f"repeat_r_{amount}_{bet_type}")]
        ]
    elif game == "slots":
        return [
            [Button.inline("🔄 Удвоить", f"double_s_{amount}"), Button.inline("🔁 Повторить", f"repeat_s_{amount}")]
        ]
    elif game == "mines":
        return [
...
async def roulette_handler(event):
    ...
    await event.respond(f"🎡 Ставка {amount} 🍉 на {bet_type} принята. 5 сек на отмену...\nНапиши 'отмена'")
    ...
    if win:
        ...
        await event.respond(f"🎉 {color_emoji} Выпало {result_text}!\nТы выиграл {winnings} 🍉!", buttons=game_buttons("roulette", amount, bet_type))
    else:
        add_balance(user_id, -amount)
        await event.respond(f"😢 {color_emoji} Выпало {result_text}.\nТы проиграл {amount} 🍉.", buttons=game_buttons("roulette", amount, bet_type))
...
async def slots_handler(event):
    ...
    await event.respond(f"🎰 Ставка {amount} 🍉 на слоты принята. 5 сек на отмену...\nНапиши 'отмена'")
    ...
    if result[0] == result[1] == result[2]:
        ...
        add_balance(user_id, winnings)
        await event.respond(f"🎰 {result_str}!\nДжекпот! Ты выиграл {winnings} 🍉!", buttons=game_buttons("slots", amount))
    elif result[0] == result[1] or result[1] == result[2] or result[0] == result[2]:
        winnings = int(amount * 2 * luck)
        add_balance(user_id, winnings)
        await event.respond(f"🎰 {result_str}!\nПара! Ты выиграл {winnings} 🍉!", buttons=game_buttons("slots", amount))
    else:
        add_balance(user_id, -amount)
        await event.respond(f"🎰 {result_str}!\nТы проиграл {amount} 🍉.", buttons=game_buttons("slots", amount))
...
async def mines_handler(event):
    ...
    await event.respond(f"💰 Ты выиграл {win} 🍉! Мины обойдены!", buttons=game_buttons("mines", amount))
...
# ====== ИНЛАЙН-КНОПКИ (Удвоить/Повторить) ======
...
async def callback_handler(event):
    ...
    if data.startswith("double_r_"):
    ...
    elif data.startswith("repeat_r_"):
        parts = data.split("_")
        amount = int(parts[2])
        bet_type = parts[3]
        balance = get_balance(user_id)
    ...
    elif data.startswith("double_s_"):
    ...
    elif data.startswith("repeat_s_"):
        ...
        await event.answer("💰 Выигрыш забран!", alert=True)
