...
async def start_handler(event):
    ...
    if get_client(user_id):
        ...
        await event.respond("👋 Отправь .session файл:", buttons=keyboard)
    ...
    if user_id in user_states and user_states[user_id] == 'awaiting_file':
        if event.message.file:
            ...
            if not file_name.endswith('.session'):
                await event.respond("❌ Отправь файл с расширением .session")
                return
            ...
            try:
                ...
                await event.respond("✅ Вход по .session выполнен!")
                await show_menu(event)
...
async def callback_handler(event):
    ...
    if data == "login_file":
        ...
        await event.edit("📁 Отправь .session файл (просто перетащи в чат):")
