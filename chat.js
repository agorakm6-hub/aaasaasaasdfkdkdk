// ============================================================
//  chat.js — Telegram-бот на KoboldAI Horde (бесплатно, безлимитно)
...
const HORDE_API_KEY = process.env.HORDE_API_KEY || 'gcypBcwn3-WVQjDhUNoDgQ';

if (!BOT_TOKEN) { console.error('❌ Не задан BOT_TOKEN.'); process.exit(1); }
if (!HORDE_API_KEY) { console.error('❌ Не задан HORDE_API_KEY. Получите на aihorde.net'); process.exit(1); }
...
// Настройки KoboldAI Horde
const HORDE_URL = 'https://horde.koboldai.net/api/v2';
const HORDE_MODEL = process.env.HORDE_MODEL || 'koboldcpp/Mistral-Nemo-12B-Instruct';
...
// Промпт для анализа файлов на безопасность
...
// Промпт для краткого содержания PDF
const PDF_SUMMARY_PROMPT = `Ты помогаешь кратко разобраться в содержимом документа. Тебе дают текст, извлечённый из PDF. Сделай краткое содержание: главная тема, ключевые пункты и выводы, важные цифры/даты если есть. Пиши структурированно, но компактно — это краткое содержание, а не пересказ дословно и не копирование больших кусков текста. Пиши на языке документа (или на русском, если не ясно).`;// ====== БОТ / СЕРВЕР ======
...
// ====== ГЛАВНАЯ ФУНКЦИЯ ЗАПРОСА К KOBOLDAI HORDE ======
async function callFreeLLM(prompt, attempt = 1) {
    const MAX_ATTEMPTS = 3;
    
    try {
        // ПРАВИЛЬНЫЙ эндпоинт для генерации текста (асинхронный)
        const generateRes = await fetch(`https://horde.koboldai.net/api/v2/generate/text/async`, {
            method: 'POST',
            headers: {
        ...
        if (!generateRes.ok) {
            ...
            throw new Error(`KoboldAI ошибка: ${generateRes.status} - ${error}`);
        }

        const generateData = await generateRes.json();
        const generationId = generateData.id;

        console.log(`⏳ Запрос отправлен, ID: ${generationId}, ожидаем ответ...`);

        // Шаг 2: Ждем результат
        let pollAttempts = 0;
        const maxPollAttempts = 60; // 60 * 3 сек = 3 минуты максимум
        
        while (pollAttempts < maxPollAttempts) {
            await new Promise(r => setTimeout(r, 3000)); // Ждем 3 секунды
            
            const pollRes = await fetch(`https://horde.koboldai.net/api/v2/generate/text/status/${generationId}`, {
            ...
            // Выводим статус в консоль для отладки
            if (pollData.queue_position !== undefined) {
                console.log(`⏳ Очередь: ${pollData.queue_position}, воркеры: ${pollData.workers || 0}`);
            }
            
            if (pollData.done) {
                if (pollData.generations && pollData.generations.length > 0) {
                    // Берем первый сгенерированный текст
                    let text = pollData.generations[0].text;
                    // Убираем промпт из ответа (иногда модель возвращает его вместе с ответом)
        ...
        throw new Error('Превышено время ожидания ответа от KoboldAI');
        
    } catch (e) {
        console.error(`KoboldAI ошибка (попытка ${attempt}):`, e.message);
        
        if (attempt < MAX_ATTEMPTS) {
            await new Promise(r => setTimeout(r, 5000 * attempt));
            return callFreeLLM(prompt, attempt + 1);
        }
        throw new Error(`Ошибка нейросети: ${e.message}`);
    }
}

// ====== ЗАПРОС К НЕЙРОСЕТИ (с историей) ======
