...
const GEMINI_IMAGE_MODEL = process.env.GEMINI_IMAGE_MODEL || 'gemini-3.1-flash-image'; // "Nano Banana"
...
async function generateImage(prompt) {
    ...
    const candidate = data.candidates?.[0];
    if (candidate?.finishReason === 'SAFETY' || candidate?.finishReason === 'PROHIBITED_CONTENT') return { blocked: true };

    const parts = candidate?.content?.parts || [];
    const imgPart = parts.find((p) => p.inlineData || p.inline_data);
    if (!imgPart) return { blocked: false, noImage: true };

    const inline = imgPart.inlineData || imgPart.inline_data;
    const buffer = Buffer.from(inline.data, 'base64');
    const textPart = parts.find((p) => p.text);
    return { buffer, caption: textPart?.text };
}

// ====== ТРИГГЕРЫ ГЕНЕРАЦИИ ИЗОБРАЖЕНИЯ (обычные фразы, без команд) ======
const IMAGE_TRIGGERS = [
    'сгенерируй изображение', 'сгенерируй картинку', 'сгенерируй фото',
    'создай изображение', 'создай картинку', 'создай фото',
    'нарисуй мне', 'нарисуй', 'сделай изображение', 'сделай картинку',
    'generate an image', 'generate image', 'generate a picture', 'draw me', 'draw'
].sort((a, b) => b.length - a.length);

function extractImagePrompt(text) {
    const lower = text.toLowerCase();
    for (const trig of IMAGE_TRIGGERS) {
        if (lower.startsWith(trig)) {
            return text.slice(trig.length).replace(/^[,:\s-]+/, '').trim();
        }
    }
    return null; // это не запрос на картинку
}
...
bot.onText(/\/start/, (msg) => {
    ...
    const text = `👋 Привет! Я AI-бот на нейросети Gemini.\n\n` +
        ...
        `🖼 Генерация картинок — просто напиши "нарисуй ...", "сгенерируй изображение ..." или "создай картинку ...", например:\n"нарисуй рыжего кота в очках"\n(в группе — не забудь добавить "${TRIGGER_WORD}" перед фразой)\n\n` +
...
bot.on('message', async (msg) => {
    ...
    // ---- запрос на генерацию изображения (по обычной фразе, без команды) ----
    const imgPrompt = extractImagePrompt(question);
    if (imgPrompt !== null) {
        if (!imgPrompt) {
            await bot.sendMessage(chatId, '✍️ Что нарисовать? Например: "нарисуй рыжего кота в очках"', isGroup ? { reply_to_message_id: msg.message_id } : {});
            return;
        }
        ...
        try {
            ...
            if (result.blocked) {
                await bot.sendMessage(chatId, '🚫 Не могу сгенерировать это изображение — запрос не проходит по правилам безопасности.', isGroup ? { reply_to_message_id: msg.message_id } : {});
                return;
            }
            if (result.noImage) {
                await bot.sendMessage(chatId, '❌ Не получилось сгенерировать, попробуй переформулировать запрос.', isGroup ? { reply_to_message_id: msg.message_id } : {});
                return;
            }
            await bot.sendPhoto(chatId, result.buffer, {
                caption: result.caption ? result.caption.slice(0, 1000) : undefined,
