...
async function callFreeLLM(prompt, attempt = 1) {
    ...
    try {
        ...
        const generateRes = await fetch(`${HORDE_URL}/generate/text`, {
            method: 'POST',
            headers: {
        ...
        let attempts = 0;
        const maxPollAttempts = 60; // 60 * 2 сек = 2 минуты максимум
        
        while (attempts < maxPollAttempts) {
            await new Promise(r => setTimeout(r, 2000)); // Ждем 2 секунды
            
            const pollRes = await fetch(`${HORDE_URL}/generate/text/status/${generationId}`, {
            ...
            if (pollData.done) {
                if (pollData.generations && pollData.generations.length > 0) {
                    return pollData.generations[0].text.trim();
                } else {
...
console.log('✅ Бот полностью запущен на KoboldAI Horde!');