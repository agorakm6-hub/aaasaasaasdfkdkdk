...
if (!HORDE_API_KEY) { console.error('❌ Не задан HORDE_API_KEY. Получить можно бесплатно на aihorde.net (Register)'); process.exit(1); }
...
// AI Horde — краудсорсинговая сеть волонтёрских серверов, бесплатно и без
// дневных лимитов, но асинхронная (запрос ставится в очередь, ответ собирают
// добровольцы) — поэтому генерация может занимать заметно дольше, чем у Gemini.
// Конкретную модель можно закрепить через HORDE_MODEL, иначе Horde сам
// выбирает любой доступный воркер.
const HORDE_BASE = 'https://aihorde.net/api/v2';
const HORDE_MODEL = process.env.HORDE_MODEL || null;
...
async function callFreeLLM(prompt, attempt = 1) {
    ...
    try {
        ...
        submitRes = await fetch(`${HORDE_BASE}/generate/text/async`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'apikey': HORDE_API_KEY, 'Client-Agent': 'melonAI:1.0:t.me/dalscam' },
        ...
        console.error(`callFreeLLM (AI Horde) network error (попытка ${attempt}):`, e.message, e.cause || '');
        if (attempt < MAX_ATTEMPTS) {
            await new Promise((r) => setTimeout(r, 1000 * attempt));
            return callFreeLLM(prompt, attempt + 1);
        }
        throw new Error('Не получилось связаться с нейросетью (AI Horde недоступен). Попробуй ещё раз через минуту.');
    }

    if (submitRes.status === 429) throw new Error('AI Horde: слишком много запросов одновременно, подожди немного и попробуй снова.');
    if (submitRes.status === 401 || submitRes.status === 403) {
        const errText = await submitRes.text();
        throw new Error(`AI Horde ${submitRes.status}: проверь HORDE_API_KEY (${errText.slice(0, 200)})`);
    }
    if (!submitRes.ok) {
        const errText = await submitRes.text();
        if (attempt < MAX_ATTEMPTS && submitRes.status >= 500) {
            await new Promise((r) => setTimeout(r, 1000 * attempt));
            return callFreeLLM(prompt, attempt + 1);
        }
        throw new Error(`AI Horde ${submitRes.status}: ${errText.slice(0, 300)}`);
    }

    const submitData = await submitRes.json();
    const genId = submitData.id;
    if (!genId) throw new Error(`AI Horde не вернул ID генерации: ${JSON.stringify(submitData).slice(0, 200)}`);

    // Horde асинхронный — запрос забирает случайный волонтёрский воркер, ответ
    // готовится не мгновенно. Опрашиваем статус, пока не будет готово.
    const POLL_INTERVAL_MS = 3000;
    const MAX_WAIT_MS = 120000; // 2 минуты — если дольше, воркеров сейчас мало
    ...
    while (Date.now() - startedAt < MAX_WAIT_MS) {
        ...
        let statusRes;
        try {
            statusRes = await fetch(`${HORDE_BASE}/generate/text/status/${genId}`, {
                headers: { 'apikey': HORDE_API_KEY },
            });
        } catch (e) {
            continue; // временный сетевой сбой при опросе — пробуем на следующем круге
        }
        if (!statusRes.ok) continue;

        const statusData = await statusRes.json();
        if (statusData.faulted) throw new Error('AI Horde: генерация завершилась с ошибкой на стороне воркера (faulted). Попробуй ещё раз.');
        if (statusData.done) {
            const gens = statusData.generations || [];
            const text = gens.map((g) => g.text || '').join('').trim();
            if (!text) throw new Error('AI Horde вернул пустой ответ.');
            return text;
        }
    }

    throw new Error('AI Horde не успел ответить за 2 минуты (сейчас мало доступных воркеров) — попробуй ещё раз чуть позже.');
}
