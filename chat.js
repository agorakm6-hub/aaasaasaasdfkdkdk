...
const FORBIDDEN_WORDS = [
    ...
    'нарколаборатори', 'оружие', 'оружия', 'пистолет', 'автомат калашников', 'винтовк',
...
async function callFreeLLM(prompt, attempt = 1) {
    ...
    try {
        ...
        console.error(`callFreeLLM network error (попытка ${attempt}):`, e.message, e.cause || '');
        if (attempt < MAX_ATTEMPTS) {
            await new Promise((r) => setTimeout(r, 1000 * attempt));
            return callFreeLLM(prompt, attempt + 1);
        }
        throw new Error('Не получилось связаться с нейросетью. Попробуй ещё раз через минуту.');
    } finally {
        clearTimeout(timeout);
    }

    if (res.status === 429) throw new Error('Сервис перегружен, подожди немного.');
    ...
    if (blockReason) {
        throw new Error(`Запрос заблокирован фильтром безопасности (${blockReason}).`);
    }
...
function quickHeuristics(text) {
    ...
    const checks = [
        ...
        [/new\s+Function\s*\(/i, 'создание функции из строки'],
        [/child_process/i, 'использование child_process'],
        [/require\(\s*['"]net['"]\s*\)/i, 'работа с сокетами'],
        [/atob\s*\(|Buffer\.from\([^,]+,\s*['"]base64['"]\)/i, 'декодирование base64'],
        [/powershell|cmd\.exe|\/bin\/sh|\/bin\/bash/i, 'запуск команд оболочки'],
        [/curl\s|wget\s|http\.get\(|https\.get\(|fetch\(/i, 'сетевые запросы'],
        [/process\.env/i, 'обращение к переменным окружения'],
        [/fs\.(unlink|rm|writeFile).*(system32|\/etc\/|\.ssh)/i, 'изменение системных файлов'],
        [/\\x[0-9a-f]{2}(\\x[0-9a-f]{2}){10,}/i, 'hex-последовательности (обфускация)'],
    ];
...
async function analyzeFileContent(fileName, text) {
    ...
    const hint = heuristics.length
        ? `Найдены подозрительные признаки: ${heuristics.join(', ')}. Учти это в анализе.`
        : 'Автоматических подозрительных признаков не найдено.';
...
const REVIEW_TRIGGERS = [
    ...
    'проверь логику кода', 'посмотри код', 'оцени код',
];
...
async function generateCodeFiles(requestText, existingFiles) {
    ...
    for (let attempt = 1; attempt <= CODE_GEN_MAX_ATTEMPTS; attempt++) {
        ...
        if (!files.length) {
            feedback = 'ответ пришёл не в формате "ФАЙЛ: имя" + блок кода';
            continue;
        }
...
async function handleSettingsCallback(query, chatId, data) {
    ...
    if (parts[1] === 'mode' && parts[2]) {
        ...
        await bot.editMessageText(
            `🔞 Этот режим только для взрослых. Напиши ответ на задание прямо сюда в чат:\n\n${question}`,
            {
                chat_id: chatId,
                message_id: query.message.message_id,
...
bot.onText(/\/start/, (msg) => {
    ...
    const text = `👋 Привет, ${name}! Я <b>melonAI</b> — бот на бесплатной нейросети.\n\n` +
        `📖 <b>Как пользоваться:</b>\n\n` +
        `💬 В личке — просто пиши что угодно.\n\n` +
        `👥 В группе — начинай сообщение со слова «${TRIGGER_WORD}»\n\n` +
        `⚙️ /settings — режим общения, голос\n` +
        `🧹 /reset — очистить память\n` +
        `🔊 «озвучь текст» — голосовое\n` +
        `🎙 /voicemode — голосовые ответы\n` +
        `🛡 Файл — анализ на безопасность\n` +
        `🔍 Ревью кода — проверка качества\n` +
        `📄 PDF — краткое содержание\n` +
        `🖼 Скриншот — OCR + исправление кода\n` +
        `💻 «напиши код» — генерация файлов\n\n` +
        `👤 Создатель: @dalscam`;
...
bot.onText(/\/voice(?:\s+([\s\S]+))?/, async (msg, match) => {
    ...
    if (!text) {
        await bot.sendMessage(chatId, 'Напиши текст после команды: /voice текст');
        return;
    }
    ...
    try {
        ...
        await bot.sendMessage(chatId, '❌ Не получилось сделать голосовое.');
    }
});
...
bot.onText(/\/voicemode/, async (msg) => {
    ...
    if (voiceMode.has(chatId)) {
        ...
        await bot.sendMessage(chatId, '🔇 Голосовые ответы выключены.');
    } else {
        voiceMode.add(chatId);
        await bot.sendMessage(chatId, '🔊 Голосовые ответы включены.');
    }
});
...
bot.on('callback_query', async (query) => {
    ...
    try {
        ...
        if (data === 'adm:menu') {
            ...
            if (u) {
                ...
                try {
                    await bot.sendMessage(
                        Number(userId),
                        u.banned ? '🚫 Ты заблокирован.' : '✅ Бан снят.'
                    );
                } catch (e) {}
            }
            await bot.answerCallbackQuery(query.id, {
                text: u ? (u.banned ? 'Забанен' : 'Разбанен') : 'Не найден',
            });
            await bot.editMessageReplyMarkup(
            ...
            await bot.answerCallbackQuery(query.id);
            await bot.sendMessage(chatId, `✍️ Напиши текст — отправлю пользователю ${userId}.`);
            ...
            await bot.answerCallbackQuery(query.id);
            await bot.sendMessage(chatId, '✍️ Отправь текст рассылки.');
        }
    } catch (e) {
...
bot.on('document', async (msg) => {
    ...
    if (doc.file_size && doc.file_size > MAX_FILE_BYTES) {
        await bot.sendMessage(chatId, `⚠️ Файл больше 2 МБ.`);
        return;
    }
    ...
    try {
        ...
        if (isPdf) {
            ...
            if (!pdfText.trim()) {
                await bot.sendMessage(chatId, '📄 Не получилось извлечь текст из PDF.');
                return;
            }
            const summary = await summarizePdf(doc.file_name || 'file.pdf', pdfText);
            await bot.sendMessage(chatId, `📄 Содержание:\n\n${summary}`);
            return;
        }
        ...
        if (nonPrintableRatio > 0.3) {
            await bot.sendMessage(chatId, '📄 Бинарный файл — анализ не поддерживается.');
            return;
        }
        ...
        if (wantsReview) {
            ...
            await bot.sendMessage(chatId, `🔍 Ревью:\n\n${review}`);
        } else {
            const analysis = await analyzeFileContent(doc.file_name || 'file', text);
            await bot.sendMessage(chatId, `🛡 Анализ:\n\n${analysis}`);
        }
    } catch (e) {
    ...
    try {
        ...
        if (lastCodeFiles.has(chatId)) {
            ...
            if (result.refused) {
                ...
                await bot.sendMessage(chatId, `⚠️ Код весит ${result.sizeKb} КБ — больше лимита.`);
...
bot.on('message', async (msg) => {
    ...
    if (isAdmin(chatId) && awaitingBroadcast.has(chatId)) {
        ...
        await bot.sendMessage(chatId, `📢 Рассылка: доставлено ${sent}, не доставлено ${failed}.`);
        return;
    }

    // Админ: сообщение пользователю
    if (isAdmin(chatId) && awaitingDirectMessage.has(chatId)) {
        ...
        try {
            ...
            await bot.sendMessage(chatId, `✅ Отправлено ${targetId}.`);
        } catch (e) {
