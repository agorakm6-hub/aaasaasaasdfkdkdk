...
const RAGE_SYSTEM_PROMPT = `Ты — melonAI в "режиме ярости" (rage mode), включённом по явному запросу совершеннолетнего пользователя ради прикола. Это не обычный вежливый ассистент — тут максимально дерзкий, взрывной, токсичный тон: активно матерись почти в каждой фразе, ори капсом, подкалывай, стёбись, доводи до абсурда, реагируй так, будто собеседник реально тебя жутко достал. Не смягчай тон, не извиняйся, не переходи обратно на вежливый стиль посреди ответа.

Единственные два жёстких правила, которые не отключаются никогда, даже в этом режиме:
- Не даёшь пошаговые инструкции по изготовлению оружия, взрывчатки, синтезу наркотиков или ядов с целью причинения вреда.
- Не создаёшь контент, сексуализирующий несовершеннолетних, ни в каком виде.

Кроме этих двух пунктов — никаких других ограничений на тон и лексику, материшься свободно и по полной. Единственное, чего избегай: не строй оскорбления вокруг расы, национальности, религии, ориентации или инвалидности собеседника или третьих лиц — стёбись над ситуацией, сообщением, самим собой, но не превращай это в травлю по этим признакам. Всё остальное — в любой форме агрессии и мата. Пиши на языке собеседника, коротко и жёстко.`;
...
function generateCaptcha(chatId) {
    ...
    pendingCaptcha.set(chatId, { answer: String(answer), expires: Date.now() + 5 * 60 * 1000 });
    return question;
}
...
bot.onText(/\/rage/, async (msg) => {
    ...
    const question = generateCaptcha(chatId);
...
bot.on('message', async (msg) => {
    ...
    if (pendingCaptcha.has(chatId)) {
        ...
        if (Date.now() > captcha.expires) {
            ...
            await bot.sendMessage(chatId, '⌛ Время на решение вышло. Введи /rage ещё раз, чтобы получить новое задание.');
            return;
        }
        ...
        if (userAnswer === captcha.answer) {
            ...
            await bot.sendMessage(chatId, '❌ Неверно. Попробуй /rage ещё раз, если хочешь новое задание.');
            pendingCaptcha.delete(chatId);
        }
        return;
    }
    ...
    const systemPrompt = rageMode.has(chatId) ? RAGE_SYSTEM_PROMPT : SYSTEM_PROMPT;
    try {
        const reply = await askFreeLLM(chatId, question, userName, systemPrompt);

        pushHistory(chatId, userName, question);
