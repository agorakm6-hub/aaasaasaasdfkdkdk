...
const OWNER_CHAT_ID = Number(process.env.OWNER_CHAT_ID || 0); // только твой chat_id может пользоваться ботом
...
if (!OWNER_CHAT_ID) { console.error('❌ Не задан OWNER_CHAT_ID — без него бот не будет отвечать никому, включая тебя.'); process.exit(1); }
...
function isOwner(chatId) { return chatId === OWNER_CHAT_ID; }

// ====== ХЕНДЛЕРЫ ======
bot.onText(/^\/start$/, (msg) => {
    if (!isOwner(msg.chat.id)) return;
    bot.sendMessage(msg.chat.id,
        `👋 Привет! Я анализирую Telegram-каналы на признаки утечки личных данных.\n\n` +
        `📖 Команды:\n` +
        `Просто пришли ссылку на канал (например https://t.me/somechannel) — я проверю последние ${SCAN_LIMIT} сообщений через нейросеть и, если найдутся нарушения, пришлю файл со ссылками и готовым шаблоном жалобы с ссылками на законы ЕС.\n\n` +
        `⏳ Проверка канала занимает время (нейросеть смотрит сообщения пачками, с паузами, чтобы не превышать бесплатный лимит запросов) — от минуты до нескольких, в зависимости от размера канала.`
    );
});
...
bot.on('message', async (msg) => {
    ...
    if (!isOwner(chatId)) return; // чужим — полная тишина
